      ...
      <el-form ref="userForm" :model="params">
        ...
        <el-form-item class="input-prepend restyle no-radius" prop="code" :rules="[{ required: true, message: '请输入验证码', trigger: 'blur' }]">
          ...
          <div class="btn" style="position:absolute;right: 0;top: 6px;width: 40%;">
            <a href="javascript:" type="button" @click="sendMsg()" :value="codeTest" disabled='ture' style="border: none;background-color: none">{{codeTest}}</a>
          </div>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       //通过输入手机号发送验证码
       sendMsg() {
         registerApi.sendMsg(this.params.mobile)
          .then(response => {
              this.sending = false
              //调用倒计时的方法
              this.timeDown()
