...
export default {
  ...
  methods:{
    ...
    //1 查询第一页数据
    initCourseFirst() {
      courseApi.getCourseList(1,8,this.searchMap).then(response => {
    ...
    //3 分页切换的方法
    gotoPage(page) {
      courseApi.getCourseList(page,8,this.searchMap).then(response => {
        this.data = response.data.data
      })
    },
    ...
    searchOne(subjectParentId,index) {
      ...
      this.twoIndex = -1
      this.searchMap.subjectId = ""
      this.subjectTwoList = []

      //把一级分类点击id值，赋值给searchMap
      this.searchMap.subjectParentId = subjectParentId
      //点击某个一级分类进行条件查询
      this.gotoPage(1)
    ...
    searchTwo(subjectId,index) {
      //把index赋值,为了样式生效
      this.twoIndex = index
      //把二级分类点击id值，赋值给searchMap
      this.searchMap.subjectId = subjectId
      //点击某个二级分类进行条件查询
      this.gotoPage(1)
    },
    ...
    searchBuyCount() {
      ...
      //把值赋值到searchMap
      this.searchMap.buyCountSort = this.buyCountSort
      this.searchMap.gmtCreateSort = this.gmtCreateSort;
      this.searchMap.priceSort = this.priceSort;
